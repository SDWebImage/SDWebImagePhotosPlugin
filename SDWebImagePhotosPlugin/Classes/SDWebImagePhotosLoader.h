...
// @note And it's also strongly recommeded to totally disable memory cache if you want to query batch of same Photos URLs frequently. You can do this by using a standalone cache which `shouldCacheImagesInMemory` is disabled. This is because Photos framework manage the image cache by their own process outside your application process and can reduce memory usage.

@interface SDWebImagePhotosLoader : NSObject <SDWebImageLoader>
